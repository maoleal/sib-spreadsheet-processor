...
import net.sibcolombia.sibsp.model.RecordFilter.FilterTime;
...
public class GenerateDwca extends ReportingTask implements Callable<Integer> {
  ...
  public GenerateDwca(@Assisted Resource resource, @Assisted ReportHandler handler, DataDir dataDir,
    ...
    super(1000, resource.getUniqueID().toString(), handler);
   ...
   */
  private void addDataFile(List<ExtensionMapping> mappings, String coreRowType) throws IOException, GeneratorException,
          ...
          if (pm.getIndex() != null) {
            ...
            if (pm.getIndex() >= 0
              && ((coreRowType.equalsIgnoreCase(Constants.DWC_ROWTYPE_OCCURRENCE) && !pm.getTerm().qualifiedName()
                .equalsIgnoreCase(Constants.DWC_OCCURRENCE_ID)) || (coreRowType
                .equalsIgnoreCase(Constants.DWC_ROWTYPE_TAXON) && !pm.getTerm().qualifiedName()
  ...
  private void addEmlFile() throws IOException {
    ...
    FileUtils.copyFile(dataDir.resourceEmlFile(resource.getUniqueID().toString(), null), new File(dwcaFolder,
  ...
  private void bundleArchive() throws IOException {
    ...
    // move to data dir
    checkForInterruption();
    File target = dataDir.resourceDwcaFile(resource.getUniqueID().toString());
    if (target.exists()) {
      target.delete();
    }
  ...
  public Integer call() throws Exception {
    try {
      ...
      addMessage(Level.INFO, "Archive generation started for resource " + resource.getUniqueID().toString());
  ...
  private void dumpData(Writer writer, ArchiveFile dataFile, ExtensionMapping mapping, int dataFileRowSize)
    ...
    try {
      File logFile = dataDir.resourcePublicationLogFile(resource.getUniqueID().toString());
      FileUtils.deleteQuietly(logFile);
      BufferedWriter logWriter = new BufferedWriter(new FileWriter(logFile));
      logWriter.write("Log Messages for publishing resource " + resource.getUniqueID().toString() + " version "
